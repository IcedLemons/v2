...
module.exports = {
    ...
    run: async ({client, message, args}) => {
        ...
        if (!args[0]) return message.reply({ embeds: [missinguser] });
        	...
        	// if (toBan.highestRole.position >= message.member.highestRole.position) return message.channel.send("The user has greater role than you");

        if (!message.member.permissions.has("BAN_MEMBERS")) return message.channel.send({ embeds: [user] });
        if (!message.guild.me.permissions.has("BAN_MEMBERS")) return message.reply({ embeds: [bot] });
	    ...
	    .setColor('#FEE75C')
	    .setTitle('Success\n')
	    .setDescription (`${member} just got banned.\n\n**REASON:** ${reason}`)
	    .setTimestamp()
	    .setFooter({ text: 'Lemonade', iconURL: 'https://lemonade.ga/noods/ApplicationFrameHost_ULyyoL2sv1.png' });
